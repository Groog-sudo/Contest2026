...
export default function KnowledgeBasePanel() {
    ...
    try {
      ...
      setMessage(
        response.status === 'accepted'
          ? `운영 지식 문서가 접수되었습니다. document_id=${response.document_id}`
          : '문서는 접수되었지만 임베딩/벡터 저장소 연결 전이라 실제 색인은 대기 상태입니다.',
      );
      setFile(null);
    } catch (caughtError) {
      if (caughtError instanceof ApiRequestError) {
        ...
        setError('운영 지식 자료 업로드 중 오류가 발생했습니다.');
      }
    } finally {
  ...
  return (
    <section className="rounded-[28px] border border-white/10 bg-white/8 p-6 shadow-xl shadow-slate-950/20 backdrop-blur">
      <h2 className="text-xl font-semibold text-white">분류 지식베이스 업로드</h2>
      <p className="mt-2 text-sm leading-6 text-slate-300">
        상담사가 참고할 CS 가이드, 배달 정책, 환불 규정, FAQ 문서를 업로드하는 영역입니다.
      </p>
      ...
      <div className="mt-4 rounded-2xl border border-white/8 bg-slate-950/30 p-4 text-sm leading-6 text-slate-300">
        {error ? (
          ...
          <p>
            분류 정확도를 높이는 운영 플레이북과 정책 문서를 연결하는 확장 지점입니다.
          </p>
        )}
      </div>
    </section>
