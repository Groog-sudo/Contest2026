...
class MentoringRepository:
    def __init__(self, settings: Settings) -> None:
        if settings.database_backend == "postgresql":
            self._backend = _PostgresMentoringRepository(
                database_url=settings.database_url,
                embedding_dimensions=settings.openai_embedding_dimensions,
            )
        else:
            self._backend = _SQLiteMentoringRepository(db_path=settings.app_db_path)
...
class _SQLiteMentoringRepository:
...
class _PostgresMentoringRepository:
            ...
            for index, chunk in enumerate(chunks):
                cursor.execute(
                    ...
                    (
                        ...
                        index,
                        chunk,
                        _vector_literal(embeddings[index]),
                        _utc_now_dt(),
                    ),
                )
            connection.commit()
